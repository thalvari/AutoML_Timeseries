...
def run_cmd(automl_name, train_path, n_pred, dt, target, time_limit_min, lang="python", verbose=False):
    ...
    log_path = f"logs_test/{Path(train_path).stem}_{automl_name}_{time_limit_min}_min_{unique_id}.log"
    pred_path = f"pred_test/{Path(train_path).stem}_{automl_name}_{time_limit_min}_min_{unique_id}.csv"

    if lang == "python":
        cmd = f"conda run -n {automl_name} bash -c 'python -u {automl_name}_test.py " \
